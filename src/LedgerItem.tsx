...
function LedgerItem({data, isCollected, isHidden, onClick, onDoubleClick}: Props) {
    const classNames = [
        ...
        data.attributes.premium ? styles.ArtifactPremium : null,
    ...
    return (
        <div className={classNames} onClick={onClick} onDoubleClick={onDoubleClick}>
            ...
            <div className={styles.ArtifactInfo}>
                ...
                <div className={styles.ArtifactItemType}>{data.attributes.itemType} | {data.attributes.claim}</div>
                <div className={styles.ArtifactClaimDescription}>{composeDescription(data.attributes)}</div>
            </div>
            <div className={styles.ArtifactIcons}>
                <span className={styles.ArtifactIcon + ' ' + styles.ArtifactIconPremium}>
                    <svg fill="currentColor" stroke="currentColor" strokeWidth="0" aria-hidden="true" version="1.1"
