...
function getDefaultItemFromCollectionItems(
...
export function aggregateItemTypes(config: Configuration): string[] {
  return Array<string>()
    ...
    .concat(
      config.showHorseArmor ? itemGroups.get(ItemGroup.HORSE_ARMOR) ?? [] : [],
    )
    .concat(config.showTrophies ? itemGroups.get(ItemGroup.TROPHIES) ?? [] : [])
    .concat(
      config.showBackTrophies
    ...
    .concat(
      config.showTownPortals
        ? itemGroups.get(ItemGroup.TOWN_PORTALS) ?? []
        : [],
    )
    .concat(
      config.showHeadstones ? itemGroups.get(ItemGroup.HEADSTONES) ?? [] : [],
    )
    .concat(config.showEmblems ? itemGroups.get(ItemGroup.EMBLEMS) ?? [] : [])
    .concat(
      config.showPlayerTitles
        ? itemGroups.get(ItemGroup.PLAYER_TITLES) ?? []
        : [],
    )
